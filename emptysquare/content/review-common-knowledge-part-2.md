...
Two generals are encamped on hilltops, on either side of the enemy army. If they attack simultaneously, they'll win, otherwise they'll fail. They have no initial plan, so they try to agree on a time to attack by sending messengers back and forth. Unfortunately, messengers can be unpredictably slow, or they can be caught by the enemy. How can the generals agree on a time?

(The paper's authors, Halpern and Moses, call this problem "coordinated attack" and they say it was introduced by Jim Gray in 1978. The puzzle may be older than that. Sometimes it's called "Chinese generals." The more complex version that Lamport named the "Byzantine generals problem" came later.)
...
General _A_'s knowledge is the set of facts that are true in all the possible scenarios that match his observations. So _A_ knows his message was sent, but that's it. If _B_ receives the message and sends an acknowledgment, she also has four scenarios she can't distinguish. The generals can't be sure they've agreed to attack unless _A_ knows that _B_ knows that ... infinitely. In other words, their attack time must be common knowledge, and common knowledge is impossible in an asynchronous system.
...
Halpern and Moses more formally define a processor's _view_ of the system, and how that relates to levels of knowledge, like distributed knowledge and common knowledge. Section 6 of the paper is great, go read it.
...
Can we do the same for Raft? The real Raft TLA+ spec has dozens of variables. Let's simplify. Let's say there's one log entry, there's a permanent leader _p_, and there are two followers _f_<sub>1</sub> and _f_<sub>2</sub>. Here are our variables:

* _r_<sub>1</sub>, _r_<sub>2</sub>: Follower _f_<sub>1</sub> or _f_<sub>2</sub> has received the entry.
* _a_<sub>1</sub>, _a_<sub>2</sub>: The leader received an acknowledgment from _f_<sub>1</sub> or _f_<sub>2</sub>.
...
As I said earlier, "the leader knows \(\psi\)" is equivalent to "the leader knows that at least one follower knows \(\varphi\)." Halpern and Moses describe how to evaluate a property like this with the indistinguishability graph. The leader knows _f_<sub>1</sub> knows \(\varphi\) at a state _S_, if every path from _S_ along zero or one leader (green) edges, then zero or one _f_<sub>1</sub> edges (blue), leads to a state where _f_<sub>1</sub> knows \(\varphi\):

![](leader-knows-f1-knows-phi.svg)

This is a graph-style way of saying: the leader knows _f_<sub>1</sub> knows \(\varphi\) in state _S_, if in all states *T* indistinguishable (to the leader) from _S_, _f_<sub>1</sub> knows \(\varphi\) in all states indistinguishable (to _f_<sub>1</sub>) from _T_&nbsp;!
...
As you'd expect, it's the states where _a_<sub>1</sub> or _a_<sub>2</sub> is true; i.e. states where one or the other follower has acknowledged the entry. But it's cool to see how it can be expressed as a graph query. This seems ripe for automatic verification.
